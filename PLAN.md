...
- Implementation & Migration plans (PLAN.md §28-29): 12-implementation/{implementation-plan,migration-plan,rollout-plan,risks,training-plan}.md. implementation-plan.md maps PLAN.md §28's 9-phase structure (0-8) onto this project's own real deliverables (Phases 0-5 done and linked directly; Migration/Pilot/Rollout notional, since there's no real legacy system or users) with a duration estimate (~19-32 weeks end to end). migration-plan.md's data inventory/mapping/cleansing/validation sections each point to real, already-proven tooling (e.g. 06-python/mappings.py's fuzzy vendor matching as the actual legacy-vendor-mapping mechanism) rather than describing migration in the abstract. rollout-plan.md makes and states an explicit assumption (pilot on Northstar Shared Services Ltd — GBP-only, no FX risk, the entity the Shared Services team already operates) since no real per-entity data exists to base it on, sequences the US entity last specifically to de-risk FX logic, and defines a per-entity (not group-wide) rollback trigger. risks.md is a 10-item register spanning data migration/control design/change management/technical risk, each tied to a specific mitigation already built or explicitly planned. training-plan.md is honestly scoped as a plan, not delivered training, drawing on the controls.md/README.md documentation already written across every phase. Pure documentation, no code — verified via a relative-link check confirming every cross-reference across all 5 files resolves to a real file.
- Solutions-consulting demo (PLAN.md §34-35): 11-demo/{demo-script,demo-flow,discovery-demo}.md — the final item of Milestone 8. demo-script.md follows PLAN.md §34's exact 10-minute timed structure, with every `[SHOW: ...]` cue pointing at a real, working artifact (a specific messy invoice row in 04-data/invoices.csv, a real rule-findings.md finding, the live dashboard) rather than a mockup, and includes the honest EVAL-010 "wasn't caught" disclosure at 6:00 rather than only the confident parts. demo-flow.md is the technical run-of-show (pre-demo setup commands, which tab is open when, static-file fallbacks if a live command misbehaves). discovery-demo.md makes PLAN.md §35's "you told me X → we found Y → we designed Z → here's the control model → here's the implementation" chain explicit for all five current-state pain points, each with real citations end to end (discovery-notes.md finding → process-analysis.md root cause → architecture/ADR decision → the actual built feature → its controls.md → its place in the implementation/rollout plan). Pure documentation, no code — link-checked. This completes Milestone 8 (Productisation) in full.
- Milestone 9 (Portfolio) — PLAN.md §49's checklist, 3 of 4 items: 13-case-study/case-study.md (§40's exact structure; Results section explicitly separates **Achieved** — real POC metrics: 128/128 tests, both hard accounting rules proven unbreakable, real reconciliation match rates, 4 real bugs found by running code — from **Target** — the R01-R06 business outcomes, which this POC cannot measure without a real deployment). 13-case-study/executive-summary.md (§41's two-page structure, written as markdown source for a PDF export rather than a binary checked into git). Root README.md fully rewritten (was still "early scaffold" placeholder) to answer all 10 PLAN.md §50 questions. All new files link-checked. **Demo video (the 4th Milestone 9 item) is a deliberate, disclosed exception** — this environment has no screen-recording capability; 11-demo/demo-script.md is the ready-to-use source for the user to record it themselves.
- KNOWN_ISSUES.md (user-requested, 2026-09-07): a single root-level document consolidating every real bug found and fixed during development (with root cause and fix, e.g. the pandas NaN-truthiness bug and the dashboard's missing-columns SQL bug), the AI evaluation's disclosed EVAL-010 gap, every documented-but-unconfirmed assumption (approval threshold, audit sample rate, outlier tolerance, fuzzy-match cutoff, AI confidence threshold), every not-yet-wired-together integration, and every deliberate scope boundary — built by grep-scanning every phase's own README.md/controls.md for its "Known limitations" section rather than from memory, so every item is sourced and cross-linked, not reconstructed. Linked from README.md and case-study.md. Link-checked; full suite re-verified 128/128 before committing.
...
- **2026-09-09: Dashboard deployed live** at https://northstar-finance-os-bndfmftzjicmew2hvehcdx.streamlit.app/ (public, no login) via Streamlit Community Cloud, connected to a free Neon Postgres database loaded with the same 05-sql/schema.sql + seed.sql dataset as local dev. No code changes were needed — 10-dashboard/db.py already read `DATABASE_URL` from the environment; the connection string is stored only in Streamlit Cloud's private app secrets, never committed to the repo. Linked from README.md (top banner + dashboard table row).
...
- NOTE ON PHASE NUMBERING: PLAN.md's own §-numbered phases (used in this STATUS block) are the authoritative sequence, and PLAN.md's §49 Milestone Checklist is the authoritative way to answer "how many phases/milestones are left" — Milestone 9 (Portfolio) is 3/4 items done (case study, executive summary, README — only demo video outstanding, deliberately, see above). The project is functionally complete. Do not use the early saved roadmap plan's (~/.claude/plans/groovy-discovering-pinwheel.md) flat "Step 0-18" numbering for progress counts — it's superseded and was a source of confusion earlier in this project.
